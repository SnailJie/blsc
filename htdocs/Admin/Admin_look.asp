...
%><form name="myform" method="post" action="Admin_Batchlook.Asp">
      <tr class="Table_row_1"> 
		...
		<input type="text" name="guestName" class="ninput" size="25" value="<%=rs("guestName")%>" id="xFilePath<%=rs("messageid")%>"><input type="button" value=" 浏览 " onclick="BrowseServer<%=rs("messageid")%>();"></td> 
...
<input type="hidden" name="gongsi" value="男" checked>
 ...
 <input type="text" name="zhiye" class="ninput" size="25" value="<%=rs("zhiye")%>"> </td> 
	...
	Else%>
      <tr> 
        <td height="25" colspan="4" align="center"><font color="#006699"><b>尚未添加内容</b></font></td>
      </tr>
      <%	End If%>
      <tr> 
